const url = 'https://api-backend-3zj7.onrender.com/main/produccion';
      ...
      .then(function (data) {
        ...
        respuesta = listaProducciones.map(function (produccion) {
          return `<tr>
              ...
              <td>
                <button type="button" class="btn btn-sm btn-info" onclick="verDetalle(${JSON.stringify(produccion)})" data-bs-toggle="modal" data-bs-target="#detalleModal">Ver detalle</button>          
  ...
  const verDetalle = async () => {
    ...
    .then(function(data) {
      let listaProducciones = data.produccion;
      listaProducciones.map(function(produccion) {
        respuesta += `<tr>
          <td>${produccion._id}</td>
          <td>${produccion.referencia}</td>
          <td>${produccion.proceso}</td>
          <td>${produccion.cantProceso}</td>
          <td>${produccion.color}</td>
          <td>${produccion.cantColor}</td>
          <td>${produccion.talla}</td>
          <td>${produccion.cantTalla}</td>
          <td>
            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="asignar(${JSON.stringify(produccion)})">Asignar</button>
          </td>
        </tr>`;
  ...
  const editar = (produccion) => {
    ...
    document.getElementById('fechaRegistro').value = produccion.fechaRegistro;
...
const registrar = async() =>{
  ...
  if (_validateCantProceso && _validateCantColor && _validateCantTalla && _validateCliente && _validateNumOrden && _validateRef && _validateProc && _validateColor && _validateTalla && _validateEstado){    
...
const Actualizar = async(id) =>{
  ...
  if (validateCantProceso && validateCantColor && validateCantTalla && validateCliente){
